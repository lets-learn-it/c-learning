  ...
  while(1) {
    ...
    while(SDL_PollEvent(&event)) {
      ...
      case SDL_KEYDOWN:
        ...
        vkey = chip8_keyboard_map(&chip8.keyboard, key);
        if (vkey != -1) chip8_keyboard_down(&chip8.keyboard, vkey);
        break;

      case SDL_KEYUP:
        key = event.key.keysym.sym;
        vkey = chip8_keyboard_map(&chip8.keyboard, key);
        if (vkey != -1) chip8_keyboard_up(&chip8.keyboard, vkey);
        break;
      }
    }
